...
const AllRooms = () => {
    ...
    const [openFilters, setOpenFitler] = useState(false);
    ...
    const handlePriceRangeChange = (isChecked, label) => {
        if (isChecked) {
            ...
            setSelectedPriceRanges(selectedPriceRanges.filter((item) => item !== label));
        }
    };
            ...
            <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
                ...
                <div className={`flex items-center justify-between px-5 py-2.5 min-lg:border-b border-gray-300 ${openFilters && 'border-b'}`}>
                    ...
                    <div className='text-xs cursor-pointer'>
                        <span
                            onClick={() => setOpenFitler(!openFilters)}
                            className='lg:hidden'>{openFilters ? 'HIDE' : 'SHOW'}
                        </span>
                        <span
                            className='hidden lg:block'
                            onClick={() => { setSelectedRoomTypes([]); setSelectedPriceRanges([]); setSelectedSortOption(""); }}
                        >CLEAR
                        </span>
                    </div>
                </div>
